 ...
 * @return {Array}
 */
ExtensionCheck.prototype.listDetails = function ( zipFilePath, fileExtension, callback ) {

    var matchingFiles = [];
    fileExtension = _.trimLeft( fileExtension, '.' );
    ...
    if ( fs.existsSync( zipFilePath ) ) {
        ...
        zipFile.forEach( function ( item ) {
            if ( !item.isDirectory && !_.isEmpty( fileExtension ) && getFileExtension( item.name ).toLowerCase() === fileExtension.toLowerCase() ) {
                matchingFiles.push( item.entryName );
            } else if ( !item.isDirectory && _.isEmpty( fileExtension ) ) {
                matchingFiles.push( item.entryName );
            }
        } );

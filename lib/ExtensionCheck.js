...
ExtensionCheck.prototype.listDetails = function ( zipFilePath, fileExtension, callback ) {
    ...
    fileExtension = _.trimLeft( fileExtension, '.' ).toLowerCase();
    ...
    if ( fs.existsSync( zipFilePath ) ) {
        ...
        zipper.unzip( zipFilePath, function ( unzipped ) {
            if ( unzipped.content && unzipped.content.files ) {
                var matchingFiles = [];
                _.each( unzipped.content.files, function ( item ) {
                    if ( !item.dir && !_.isEmpty( fileExtension ) && getFileExtension( item.name ) == fileExtension ) {
                        matchingFiles.push( item.name );
                    } else if ( !item.dir && _.isEmpty( fileExtension ) ) {
                        matchingFiles.push( item.name );
                    }
                } );
            }
